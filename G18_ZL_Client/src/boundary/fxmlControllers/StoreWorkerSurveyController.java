...
public class StoreWorkerSurveyController implements Initializable {
        ...
        private String [] questions = {"How much the service was understood to you from 1 - 10 ?","Rate how much the service was comfort from 1 - 10 " , "Rate the workers treatment from 1 - 10","How much the service was complicated in your opinion from 1 - 10 ?\n"
        		, "Rate the design of the service from 1 - 10 ","How much are you satisfied from the flowers that you bought from 1 - 10 ?\n"
        				};
        	
	@Override
	public void initialize(URL arg0, ResourceBundle arg1) {
		ArrayList<String> storesList = (ArrayList<String>) MainController.getMyClient().send(MessageType.GET, "store/all",null);
		//for(int i=0 ; i<storesList.size() ; i++)
		//storeComboBox.setValue(storesList.get(i));
		for(int i=0 ; i<6 ; i++) {
		...
		//ArrayList<SurveyQuestion> questionsList = SurveyQueryFromDB(MessageType.GET, null);
		setQuestions();
	}
	...
	private void initializeSurvey() {
		...
		survey = new Survey(MainController.currentTime(),1);
	...
	public void enableEnterAnswerButton() {
		enterSurveyButton.setDisable(!isAnsweredQuestion[0] || !isAnsweredQuestion[1] || !isAnsweredQuestion[2] || !isAnsweredQuestion[3] || !isAnsweredQuestion[4]|| !isAnsweredQuestion[5]);
	}
